...
export default function PollPage() {
    ...
    useEffect(() => {
        const fetchPollData = async () => {
            try {
                // Fetch poll data
                const pollResponse = await fetch(`http://realtime-polling.vercel.app/api/polls/${pollId}`);
                if (!pollResponse.ok) {
            ...
            try {
                ...
                for (const option in formattedData) {
                    ...
                    const userNames = await Promise.all(userIds.map(async userId => {
                        const userResponse = await fetch(`http://realtime-polling.vercel.app/api/studentauth/getusername/${userId}`);
                        if (userResponse.ok) {
