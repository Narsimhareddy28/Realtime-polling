...
window.location.href = '/studentdashboard';      }
  ...
  useEffect(() => {
    const fetchPoll = async () => {
      try {
        const response = await fetch(`http://realtime-polling.vercel.app/api/polls/${pollId}`);
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await fetch(`http://realtime-polling.vercel.app/api/comments/${pollId}`);
        if (response.ok) {
          const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://realtime-polling.vercel.app/api/comments', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchPollPercentages = async () => {
      try {
        // Fetch poll percentages
        const response = await fetch(`http://realtime-polling.vercel.app/api/pollResponses/${pollId}`);
        if (!response.ok) {
      ...
      try {
        ...
        const usernameData = await Promise.all(
          userIds.map(async userId => {
            const response = await axios.get(`http://realtime-polling.vercel.app/api/studentauth/getusername/${userId}`);
