...
const Singlepoll = () => {
  ...
  useEffect(() => {
    const fetchPoll = async () => {
      try {
        const pollResponse = await fetch(`http://realtime-polling.vercel.app/api/pollResponses/fetchpoll/${userId}/${pollId}`);
  ...
  const checkHasVoted = async (pollId) => {
    try {
      const response = await fetch(`http://realtime-polling.vercel.app/api/pollResponses/hasVoted/${userId}/${pollId}`);
      if (response.ok) {
